...
Horae 的所有視覺樣式由 **CSS 變數** 控制。只需覆蓋這些變數，即可改變整個外掛外觀。

### 方式一：修改 CSS 變數（推薦）

在外掛設定 → 外觀設定 → 客製化CSS 中輸入：
...
| 變數 | 預設值 | 說明 |
|------|--------|------|
| `--horae-radar-color` | 跟隨 `--horae-primary` | 雷達圖資料區顏色（填充/描邊/頂點） |
...
| `.horae-message-panel` | 訊息底部的後設資料面板 |
| `.horae-modal` | 所有模態彈窗 |
...
- `css`：額外的 CSS 程式碼（可選），用於無法透過變數實現的樣式調整
...
部分酒館美化或預設的 z-index 較高，導致 Horae 底部面板被蓋住。在客製化 CSS 中新增：
...
4. **不影響酒館**：Horae 的所有樣式都限定在外掛容器內，不會影響酒館主介面。
